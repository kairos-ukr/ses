      ...
      <div className="min-w-0 flex-1">
        <div className="flex justify-between items-center mb-1">
          <p className="text-sm font-bold text-slate-800 truncate">
            {item.name} <span className="text-slate-400 font-normal text-xs">#{item.custom_id}</span>
          </p>
        </div>
...
export default function HomePage() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      // 1. Installations (для "уваги")
      const installationsReq = supabase
        .from('installations')
        .select('id, custom_id, name, status, capacity_kw, workflow_stage, workflow_data')
        .neq('status', 'cancelled');

      // 2. Work Today
      const workersTodayReq = supabase
      ...
      // 3. MICROTASKS (Задачі на сьогодні або прострочені)
      ...
      const [instRes, workRes, tasksRes] = await Promise.all([
        installationsReq, workersTodayReq, tasksReq
      ]);
      ...
      // Сортуємо: Спочатку задачі, потім проекти
        ...
        <div className="flex-none flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none justify-center bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg shadow-indigo-200 transition-all active:scale-95 flex items-center gap-2"
            >
            ...
            <button
              ...
              className="flex-1 sm:flex-none justify-center bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2"
            >
              <FaTasks /> <span>Мікрозадачі</span>
            </button>
          </div>
        </div>
